...
{
    int i;
    for(i = 0; i < PWM_REG_NUM; i++)record_reg[i] = sunxi_pwm_read_reg(i * 0x4);
    return 0;
}
...
{
    int i;
    for(i = 0; i < PWM_REG_NUM; i++)sunxi_pwm_write_reg(i * 0x4, record_reg[i]);
    return 0;
}
...
static int __init pwm_module_init(void)
{
    ...
    alloc_chrdev_region(&devid, 0, 1, "my_pwm");
    my_cdev = cdev_alloc();
    ...
    pwm_class = class_create(THIS_MODULE, "my_pwm");
    if (IS_ERR(pwm_class))
    {
    ...
    pwm_dev = device_create(pwm_class, NULL, devid, NULL, "my_pwm");
